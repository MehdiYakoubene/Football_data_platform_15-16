...
def build_player_match_stats(events: pd.DataFrame) -> pd.DataFrame:
    ...
    grouped = events.dropna(subset=["player_id"]).groupby(["match_id", "team_id", "team_name", "player_id", "player_name"], dropna=False)
    stats = grouped.agg(
    ...
    return stats.sort_values(["match_id", "team_name", "player_name"])
